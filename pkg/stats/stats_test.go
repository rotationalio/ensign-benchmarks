...
func loadStandardizedData() ([]float64, error) {
...
func ExampleStatistics() {
	stats := stats.New()
	samples, _ := loadStandardizedData()
...
func TestStatistics(t *testing.T) {
	data, err := loadStandardizedData()
	require.NoError(t, err, "could not load testdata")
	...
	require.Equal(t, 411.2431340518406, stats.Total())
...
func TestStatisticsBulk(t *testing.T) {
	data, err := loadStandardizedData()
	require.NoError(t, err, "could not load test data")
...
func TestEmptyStats(t *testing.T) {
	stats := stats.New()
	require.Equal(t, uint64(0), stats.N())
	require.Equal(t, 0.0, stats.Total())
	require.Equal(t, 0.0, stats.Mean())
	require.Equal(t, 0.0, stats.StdDev())
	require.Equal(t, 0.0, stats.Variance())
	require.Equal(t, 0.0, stats.Maximum())
	require.Equal(t, 0.0, stats.Minimum())
	require.Equal(t, 0.0, stats.Range())
}
...
func TestStatisticsAppend(t *testing.T) {
	...
	t.Run("S_Empty", func(t *testing.T) {
		...
		require.InDelta(t, 9.813435, s.Mean(), 0.000009)
		require.InDelta(t, 2.184890, s.StdDev(), 0.000009)
		require.InDelta(t, 4.773745, s.Variance(), 0.000009)
		require.InDelta(t, 15.45832, s.Maximum(), 0.000009)
		require.InDelta(t, 5.512247, s.Minimum(), 0.000009)
		require.InDelta(t, 9.946079, s.Range(), 0.000009)
	})
